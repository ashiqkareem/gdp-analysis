...
def locate(userSelection):
    ...
    #return (colGDP, colAgri, colArab, colBirth, colDeath, colIndiv, colIndus, colMobile, colMort, colCrop, colPopDen,
        #colPop, colServ, colArea)
...
def displayFactorsGraph(x):

    plt.figure("GDP Factors")
    plt.suptitle("%s GDP Factors")
    for i in range(len(locate(x))):
        plt.subplot(5, 3, i + 1)
        sns.lineplot(x=locate(x)[i][0], y='GDP', data=df)
    plt.tight_layout()
    plt.show()


def displayCorrTable(x):
    df = pd.DataFrame(data=locate(x), index=[0]).T
    df.columns = ['Correlation Value']
    window = tk.Tk()
    window.title('%s GDP Factors correlation values')
    f = Frame(window)
